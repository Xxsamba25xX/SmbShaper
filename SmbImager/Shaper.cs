...
namespace SmbImager
{
   public class Shaper
   {
      ...
      public Color[,] Normalize(Color[,] src, Color[] backgroundColors, ColorConfiguration configuration)
      {
         ...
         double total = src.LongLength, partial = 0;
         for (int i = 0; i < src.GetLength(0); i++)
         {
            for (int j = 0; j < src.GetLength(1); j++)
            {
               ...
               OnProgressChanged?.Invoke(this, new Progress(total, ++partial));
            }
         }
         return result;
      }

      public List<Shape> Shapify(Color[,] src, ColorConfiguration configuration)
      {
      ...
      private (Point newActualPoint, Point newLastPoint) GetNextPoint(Point actualPoint, Point lastPoint, Color[,] src, ColorConfiguration configuration)
      {
      ...
      private bool IsOutOfBounds(Rectangle bounds, Point item)
      {
         return (item.X < bounds.Left || item.X >= bounds.Right) ||
                (item.Y < bounds.Top || item.Y >= bounds.Bottom);
      }

      private Point[] GetColliders(Point point, Cardinal from, bool fullColliders)
      {
      ...
      public bool IsBackgroundColor(Color src, Color[] background)
      {
